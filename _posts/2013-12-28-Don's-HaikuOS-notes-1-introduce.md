---  
layout: post   
title: Don's HaikuOS Notes 1:简介 v2    
summary: Haiku OS是一个开源、免费的操作系统，主要面向个人计算机，它快速、简洁、易学易用，但功能非常强大。它有：<ul><li>- 拼贴和标签式的窗口</li><li>- 快速响应的内核；</li><li>- 针对多核处理器设计</li><li>- 深入的多线程化</li><li>- 先进的全日志、数据库式64位文件系统</li><li>- 基于属性的快速索引和查询</li></ul>有人说，Haiku给人一种别样的美好感觉，她将让你重新认识个人计算机。的确，自从认识了Haiku，我就被她的别样风情和众多特性迷住了。这篇文章是Don's HaikuOS Notes系列文章的第一篇，简单介绍Haiku操作系统的历史、设计理念、各种特性及其架构。希望能让大家认识、感受到一个别样美好的操作系统。<p><center><a href="/images/haiku-notes/1_HaikuOS-desktop.png" target="_blank"><img src="/images/haiku-notes/1_HaikuOS-desktop.png" alt="HaikuOS-desktop" height="500" width="633"></a><p>Haiku OS 桌面</center><p>     
...
Haiku操作系统创建的目的是开源再现已经停止开发的BeOS操作系统。

### BeOS
BeOS操作系统([BeOS Wiki](http://en.wikipedia.org/wiki/BeOS)，[中文维基](http://zh.wikipedia.org/wiki/BeOS)，[百科](http://baike.baidu.com/link?url=RlMCN12Pq2RYX_9z1C_GsXaWVP7kK3e2SZF_PDZ6ptjrlRBS28YSQ-jUXIuAc4yx))是由Be公司开发的针对个人计算机的操作系统。1990年，前苹果公司主管Jean-Louse Gassee创办了Be公司，历经4年艰辛，开发出全新的BeOS。最初运行于自行开发的BeBox个人电脑之上。BeOS面向数字多媒体处理，采用了多处理器、多线程和抢占式多任务优化设计，使用64位日志式文件系统BFS，性能异常优越；人机界面基于清晰、干净、简洁的设计理念，易学易用。BeOS的技术和设计理念遥遥领先于同时代的操作系统。

1996年，苹果公司放弃了自己开发新的操作系统，认为BeOS符合他对操作系统的要求，开价2亿美元收购Be公司，但遭到拒绝。转而以4.29亿美元的代价迎回了之前被踢出局的苹果创始人乔布斯和他的NeXTSTEP操作系统，乔布斯再次成了苹果掌门人，NeXTSTEP成了新的Mac OS X系统的基础。而Be公司，技术的先进并不能弥补市场开发的拙略，战略几经转变，均未获市场成功，在2001年，被Palm公司收购。BeOS这个领先于时代，却又命运多舛的多媒体操作系统也走到了尽头。

### OpenBeOS / Haiku
同年，一群忠实爱好者开始了OpenBeOS开源项目，目标是重建一个与BeOS 最终版本R5兼容的开源操作系统。2003年，他们在美国纽约州成立了非营利的组织[Haiku,Inc](http://www.haiku-inc.org/) 以支持该项目。2004年，由于版权原因，项目名称改为Haiku(一种日本古典短诗-俳句的发音)。2009年9月，经过世界各地的十几个核心开发人员8年的努力，项目组终于发布了第一个测试版Haiku R1/Alpha1。目前，Haiku已经发布了4个Alpha版本，即将进入Beta发布阶段。

HaikuOS官方主页：<http://www.haiku-os.org/><p>
HaikuOS测试版本下载：<https://download.haiku-os.org/nightly-images/x86_gcc2_hybrid/><p>

## II 设计理念和特性

HaikuOS的设计理念继承于BeOS，专注于个人计算机的全图形化的“多媒体操作系统”。设计目标是尽可能的减少内核延迟，从而实时处理大量多媒体数据，如音频和视频数据流。由于没有旧系统兼容的历史负担，它充分利用了现代硬件和技术的优点，如利用模块化I/O带宽的多处理器系统、深入的多线程、抢断式的多任务和被称为BFS的数据库式、全日志式的64位文件系统等。因而，Haiku/Beos的性能异常优异，可以流畅地同时播放多个视频、音频；日志系统保证意外情况下的断点恢复，不会丢失数据；而数据库式文件系统又能快速、方便的存储、管理、查询多媒体文件的属性...

### 诸多特性
1. __统一、简洁、优雅的全图形用户界面 / unified，simple，decent graphic UI__<p>

    Haiku的图形化人机界面遵循清晰、整洁的设计原则开发，简洁、优雅、人性化，，操作非常简单，几乎没有上手难度。
	
	HaikuOS的GUI得到了内核级的深入支持，其API兼容于BeOS R5，由C++编写而成。虽非源于Unix的操作系统，但实现了POSIX兼容，并实现了Bash shell命令行。设计简洁、优雅、人性化

    Haiku的拼贴和标签式（Stack & Tile）窗口排列很有特点，可以将两个或多个窗口并排或竖排拼接或像标签一样堆叠起来作为一个整体，非常便于窗口的排列和管理。

	Haiku还提供一些钟表、计算器、音量控制等一些常用部件的桌面插件，称为Replicants，直接将Replicants面板拖到桌面，就可以嵌入桌面，与桌面壁纸融于一体，美观、方便。
	
	扩展阅读：[Haiku的图形用户界面、标签式窗口、桌面附件等的图示](http://www.haiku-os.org/docs/userguide/zh_CN/contents.html)
...
1. __内核层 / kernel__<p>
	...
	Haiku的内核是混合内核（hybrid)，驱动运行在内核中，并可动态链接；图形驱动分别位于内核和应用服务（app_server)中。除了驱动，大部分的内核属于单一内核。[5]

2. __服务层 / app_server__<p>
    内核之上是服务层。服务（server）层包含多个服务（servers），在后台处理程序请求的系统功能，如输入服务（Input Server）处理键盘、鼠标等输入设备操作；应用程序服务（Application Server）处理图形显示（Anti-Grain Geometry）、应用程序通信。各个服务都是独立的、可重启的[6]，因而某个服务崩溃不会影响到其他服务。
...
### Haiku操作系统的技术
Haiku主体由C++编写，提供了面向对象的应用程序接口（API）。BeOS模块化的设计也使得Haiku可以从互相独立的小组开始开发，编写BeOS相应模块的替代品。
...
[9][Developer interview how haiku building better BeOS](http://www.techworld.com.au/article/432871/developer_interview_how_haiku_building_better_beos/?pp=3)    
